#!/bin/bash
#############################################################################
# Tests for mqm_full_queue.sh
# Run with: bash test/test_mqm_full_queue.sh
...
test_script_has_proper_shebang() {
    local first_line
    first_line=$(head -1 "$SRC_DIR/mqm_full_queue.sh")
...
test_script_sources_common() {
    if grep -q '\. .*mqm_common\.sh\|source.*mqm_common\.sh' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_has_copyright() {
    if grep -q "COPYRIGHT Kyndryl" "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_has_version() {
    if grep -q "Version" "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_checks_for_jq() {
    if grep -q "command -v jq" "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_outputs_error_json_when_jq_missing() {
    if grep -q 'mqm_error_json.*jq' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_jq_output_produces_valid_json() {
    ...
    local result
    result=$(printf 'QM1\tMY.QUEUE\t10\t5000\nQM1\tOTHER.QUEUE\t0\t10000\n' | jq -c -R -n '
        [ inputs
          | select(length>0)
          | capture("^(?<q_manager>\\S+)\\t(?<q_name>\\S+)\\t(?<curr>\\d+)\\t(?<max>\\d+)")
          | { Q_MANAGER: .q_manager
            , Q_NAME: .q_name
            , Q_CURRDEPTH: (.curr|tonumber)
            , Q_MAXDEPTH: (.max|tonumber) }
        ]')
...
test_jq_output_has_required_fields() {
    ...
    local qm_value q_name_value curr_value max_value
    ...
    curr_value=$(echo "$result" | jq -r '.[0].Q_CURRDEPTH')
    assert_equals "10" "$curr_value" "Q_CURRDEPTH field should be 10"
    
    max_value=$(echo "$result" | jq -r '.[0].Q_MAXDEPTH')
    assert_equals "5000" "$max_value" "Q_MAXDEPTH field should be 5000"
}

test_jq_handles_zero_depth() {
    if ! command -v jq &>/dev/null; then
        echo "  (skipped - jq not installed)"
        return 0
    fi
    ...
    local result
    result=$(printf 'QM1\tEMPTY.QUEUE\t0\t10000\n' | jq -c -R -n '
        [ inputs
          | select(length>0)
          | capture("^(?<q_manager>\\S+)\\t(?<q_name>\\S+)\\t(?<curr>\\d+)\\t(?<max>\\d+)")
          | { Q_MANAGER: .q_manager
    ...
    local curr_value
    curr_value=$(echo "$result" | jq -r '.[0].Q_CURRDEPTH')
    assert_equals "0" "$curr_value" "Q_CURRDEPTH should be 0"
}
...
test_embedded_script_sets_path() {
    if grep -q 'PATH=.*\$MQM_PATH' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_embedded_script_uses_runmqsc() {
    if grep -q 'runmqsc' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_embedded_script_displays_qlocal() {
    if grep -q 'DISPLAY QLOCAL' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
        echo "  Embedded script should display QLOCAL"
        return 1
    fi
}

test_embedded_script_queries_curdepth_maxdepth() {
    if grep -q 'CURDEPTH' "$SRC_DIR/mqm_full_queue.sh" && \
       grep -q 'MAXDEPTH' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
        echo "  Embedded script should query CURDEPTH and MAXDEPTH"
        return 1
    fi
}
...
test_script_uses_temp_script_file() {
    # Script uses temp file approach for better portability
    if grep -q 'tmpscript=\|cat.*MQSC_EOF' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_handles_sudo_execution() {
    if grep -q 'sudo -n -u mqm' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_checks_current_user() {
    if grep -q 'id -un' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_has_debug_function() {
    if grep -q 'dbg()' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_checks_debug_variable() {
    if grep -q 'DEBUG' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_supports_include_system() {
    if grep -q 'INCLUDE_SYSTEM' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
test_script_filters_system_queues() {
    if grep -q 'SYSTEM\.' "$SRC_DIR/mqm_full_queue.sh"; then
        return 0
    else
...
echo ""
echo "========================================"
echo "  mqm_full_queue.sh Test Suite"
echo "========================================"
echo ""
...
run_test "JSON output: handles zero depth" test_jq_handles_zero_depth
...
run_test "embedded script: displays QLOCAL" test_embedded_script_displays_qlocal
run_test "embedded script: queries CURDEPTH/MAXDEPTH" test_embedded_script_queries_curdepth_maxdepth

# Shell compatibility tests
run_test "shell compat: uses temp script file" test_script_uses_temp_script_file
