...
# Version 1.2
#
# mqm_full_queue.sh - Monitor IBM MQ Queue Depths
#
# PURPOSE:
#   This script queries queue depth information for all local queues across
#   active queue managers and outputs the results in JSON format for Zabbix.
#
# WHAT IT DOES:
#   1. Reads active queue managers from the cache file
#   2. Runs 'runmqsc DISPLAY QLOCAL(*)' for each queue manager as mqm user
#   3. Extracts CURDEPTH and MAXDEPTH for each queue
#   4. Filters out SYSTEM.* queues and dead-letter queues (unless INCLUDE_SYSTEM=1)
#   5. Outputs JSON array with queue depth information
#
# OUTPUT FORMAT:
#   [{"Q_MANAGER":"QM1","Q_NAME":"MY.QUEUE","Q_CURRDEPTH":10,"Q_MAXDEPTH":5000}]
#
...
# --------------------------------------------------------------------------
# Create embedded script file (run as mqm user) - outputs tab-separated queue data
# Using a temp file avoids complex quoting issues with heredocs and special chars
# --------------------------------------------------------------------------
tmpscript="/tmp/mqm_full_queue_$$.sh"
tmpoutput="/tmp/mqm_full_queue_$$.out"
...
# Optional debug
dbg() { [ -n "$DEBUG" ] && echo "[DEBUG] $*" >&2; }

# Build tab-separated rows: QM QUEUE CUR MAX
ROWS=""
...
while read -r QM; do
    ...
    # Single MQSC call to fetch all local queues with depths
    MQ_OUT=$(echo "DISPLAY QLOCAL(*) CURDEPTH MAXDEPTH" | runmqsc "$QM" 2>/dev/null)
    if [ -z "$MQ_OUT" ]; then dbg "No MQ_OUT for $QM (runmqsc missing or no output)"; fi
    
    # Count QUEUE() lines; fallback to ALL if zero to handle format differences
    qcount=$(echo "$MQ_OUT" | grep -c 'QUEUE(' || true)
    if [ "$qcount" -eq 0 ]; then
        dbg "No QUEUE() lines for $QM with CURDEPTH/MAXDEPTH; trying DISPLAY QLOCAL(*) ALL"
        MQ_OUT=$(echo "DISPLAY QLOCAL(*) ALL" | runmqsc "$QM" 2>/dev/null)
        qcount=$(echo "$MQ_OUT" | grep -c 'QUEUE(' || true)
    fi
    ...
    ROWS_QM=$(echo "$MQ_OUT" | awk -v qm="$QM" -v deadq="$DEAD_QUEUE" -v includeSystem="$include_system" '
        ...
        /CURDEPTH\(/ {
            val=$0; gsub(/.*CURDEPTH\(|\).*/, "", val); gsub(/[[:space:]]+/, "", val)
            cur=val
        }
        /MAXDEPTH\(/ {
            val=$0; gsub(/.*MAXDEPTH\(|\).*/, "", val); gsub(/[[:space:]]+/, "", val)
            max=val
        }
        {
            if (name != "" && cur != "" && max != "") {
...
# Input format: QM<TAB>QUEUE<TAB>CUR<TAB>MAX
# --------------------------------------------------------------------------
